...
Rack::Attack.blocklist('block ips going to GET /saved_searches/save/') do |req|
  if req.get? && req.path.include?('/saved_searches/save/')
    Rails.cache.write("saved-search-bot:#{req.ip}", true, expires_in: 30.days)
    File.write(Rails.root.join('log/get-saved-search_blocked-ips.log').to_s, "#{req.ip} - [#{DateTime.now.strftime('%Y-%m-%dT%H:%M:%S')}] - #{req.path} - #{req.user_agent.inspect}\n", mode: 'a')
    true
  else
    false
  end
